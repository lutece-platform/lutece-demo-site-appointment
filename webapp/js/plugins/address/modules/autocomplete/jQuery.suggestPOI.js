    ...
    $.suggestPOI = function(elem, options, arg) {
	    ...
	    jElem.autocomplete({
	        ...
	        source: function(input, forward) {
                ...
                var ajax_data;
                var ajax_url;
								ajax_url = "http://geocode.arcgis.com/arcgis/rest/services/World/GeocodeServer/suggest";
								ajax_data = {
									"text":input.term,
									"f":"json",
									"location":"-76.610838,39.290736"
								}
								/*
                    ...
                    if ($config[PROP_API_INPUT] == APIINPUT_STOREADR) {
                      ...
                      $.get( ajax_url, undefined, function(data) {
                        ...
                        $(this).trigger($.Event(EVT_SELECT, ui.item));
										...
										*/
										var ajax_url ="http://geocode.arcgis.com/arcgis/rest/services/World/GeocodeServer/findAddressCandidates";
										var ajax_data = {
											"f":"json",
											"singleLine":ui.item.poi.libelleTypo,
											"magicKey":ui.item.poi.magicKey
										};
